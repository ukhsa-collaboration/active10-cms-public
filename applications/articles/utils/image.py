...
from ..settings import OPTIMIZED_IMAGE_METHOD
...
def image_optimizer(image_data, output_size=None, resize_method=None):
    ...
    if OPTIMIZED_IMAGE_METHOD == "pillow":
        ...
        if output_size is not None:
            if resize_method not in ("thumbnail", "cover", "width", None):
                message = "optimized_image_resize_method misconfigured, it's value must be 'thumbnail', 'cover' or None"  # noqa: E501
                ...
                image = resizeimage.resize_width(image, output_size[0], validate=False)

            output_image = Image.new("RGBA", image.size, BACKGROUND_TRANSPARENT)
